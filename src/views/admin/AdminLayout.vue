...
onMounted(async () => {
  ...
  local.value = l
  if (l) {
    const [s, dueño, facturacion, menu] = await Promise.all([
      soySuperAdmin(),
      soyDueñoDelLocal(l.id),
      puedoVerFacturacion(l.id),
      puedoEditarMenu(l.id),
    ])
    esSuper.value = s
...
const NAV = [
  ...
  {
    to: (slug) => `/panel/${slug}/admin/equipo`,
    label: 'Equipo',
    exact: false,
    permiso: 'dueño',
...
const navVisible = computed(() => NAV.filter((item) => !item.permiso || permisos.value[item.permiso]))
...
const tituloPagina = computed(() => {
  ...
  if (route.name === 'admin-equipo') return 'Equipo'
