-- ============================================================
-- Filtro por categoría en Reportes.
--  · reporte_local y top_productos_local suman un p_categoria_id opcional.
--  · Con p_categoria_id NULL => comportamiento actual (totales por pedido).
--  · Con p_categoria_id seteado => se mira ítem por ítem: "ventas" = monto
--    neto de los ítems de esa categoría, "pedidos" = pedidos que tuvieron
--    al menos un ítem de esa categoría, "ticket" = ventas / esos pedidos.
--  · Los ítems de combo no tienen categoría (combos.categoria_id no existe),
--    así que quedan fuera del filtro por categoría — es lo esperado.
-- ============================================================

drop function if exists reporte_local(uuid, date, date, date, date);
...
language sql stable security definer set search_path = public as $$
  ...
  base as (
    select p.id as pedido_id, p.fecha, p.total as monto
    from ped p, cfg
...
grant execute on function reporte_local(uuid, date, date, date, date, uuid) to authenticated;


drop function if exists top_productos_local(uuid, date, date);

create or replace function top_productos_local(
  p_local_id uuid,
  p_desde date,
  p_hasta date,
  p_categoria_id uuid default null
...
grant execute on function top_productos_local(uuid, date, date, uuid) to authenticated;
