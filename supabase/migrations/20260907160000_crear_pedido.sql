-- ============================================================
-- SinFila — Hito 4: función crear_pedido()
-- El cliente (sin login) crea su pedido a través de esta función en vez de
-- insertar directo en las tablas. Dos razones, las dos ya las vimos en el
-- Hito 2b:
--   1) INSERT ... RETURNING sobre "pedidos" falla por RLS para un anónimo
--      (necesita ver la fila para devolverla, y esa policy es solo staff).
--   2) Nunca hay que confiar en precios/nombres que mande el cliente: acá
--      se recalculan siempre desde productos/combos/opciones reales.
-- ============================================================
...
begin
  ...
  -- Costo de delivery: se calcula acá, nunca se toma del cliente.
  if (p_payload->>'tipo_entrega') = 'delivery' then
    if v_local.delivery_costo_modo = 'por_barrio' then
      select costo into v_costo_delivery from zonas_delivery
        where local_id = v_local.id and barrio = p_payload->'direccion'->>'barrio';
      if not found then
        raise exception 'No hacemos delivery a ese barrio';
      end if;
    else
      v_costo_delivery := v_local.delivery_costo_fijo;
    end if;
  end if;
...
-- Cualquiera puede llamar esto (es el checkout de un cliente sin cuenta).
-- La seguridad acá no es "quién sos", es "no confiar en lo que mandás":
-- precios, disponibilidad y costo de envío se recalculan todos adentro.
revoke execute on function crear_pedido(jsonb) from public;
